...
$hookmanager->initHooks(array('isiworkconnectorlist'));
...
llxHeader('', $langs->trans('isiworkconnectorList'), '', '');

//$type = GETPOST('type');
//if (empty($user->rights->isiworkconnector->all->read)) $type = 'mine';
...
$sql.= ' FROM '.MAIN_DB_PREFIX.'isiworkconnector t ';

if (!empty($object->isextrafieldmanaged))
{
    $sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'isiworkconnector_extrafields et ON (et.fk_object = t.rowid)';
}

$sql.= ' WHERE 1=1';
//$sql.= ' AND t.entity IN ('.getEntity('isiworkconnector', 1).')';
...
$formcore = new TFormCore($_SERVER['PHP_SELF'], 'form_list_isiworkconnector', 'GET');

$nbLine = !empty($user->conf->MAIN_SIZE_LISTE_LIMIT) ? $user->conf->MAIN_SIZE_LISTE_LIMIT : $conf->global->MAIN_SIZE_LISTE_LIMIT;

$r = new Listview($db, 'isiworkconnector');
echo $r->render($sql, array(
    ...
    ,'list' => array(
        'title' => $langs->trans('isiworkconnectorList')
        ,'image' => 'title_generic.png'
        ,'picto_precedent' => '<'
        ,'picto_suivant' => '>'
        ,'noheader' => 0
        ,'messageNothing' => $langs->trans('Noisiworkconnector')
	...
	,'search' => array(
		...
		,'status' => array('search_type' => isiworkconnector::$TStatus, 'to_translate' => true) // select html, la clé = le status de l'objet, 'to_translate' à true si nécessaire
	)
	,'translate' => array()
	,'hide' => array(
